...
export class OrdersService {
  ...
  async create(createOrderDto: CreateOrderDto & { client_id: number }) {
    const productsIds = createOrderDto.items.map((items) => items.product_id);
    ...
    if (products.length !== uniqueProductIds.length) {
      throw new Error(
        `Algum produto não existe. Produtos passados ${productsIds}, produtos encontados ${products.map((product) => product.id)}`,
      );
    }
